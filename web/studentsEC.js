...
function displayStudentData(students) {
    ...
    $('.editStudentDataButton').click(function() {
        upsert($(this).attr('id').replace('edit', ''));
    });
    $('.deleteStudentDataButton').click(function() {
        window.idOfStudentToRemove = $(this).attr('id').replace('delete', '');
...
$(() => {
    ...
    $('#deleteForRealButton').click(() => {
        ...
        $.ajax({
            url: `/api/v1/students/${window.idOfStudentToRemove}.json`,
            method: 'DELETE'
        });
    });
    ...
    $('#saveChangesButton').click(function() {
        ...
        if (window.currentStudentId) { //we are editing a student
            $.ajax({
                url: `/api/v1/students/${window.currentStudentId}.json`,
                data: newStudent,
                method: 'PUT',
                success: function() {
    ...
    $.ajax({
        ...
        success: function(studentIds) {
            ...
            for (let studentId of studentIds) {
                $.ajax({
                    url: `/api/v1/students/${studentId}.json`,
...
function upsert(studentEditId) {
    if (studentEditId) { //if we are editing
        ...
        $('#editStartDateInput').val(studentToEdit.startDate);
